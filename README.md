...
🌐 **Live Demo:** **[Try the App on Streamlit](https://seq2seq-french-to-english.streamlit.app/)**
...
A classical sequence-to-sequence model based on Sutskever et al. (2014). The encoder processes the French sentence and condenses it entirely into the final hidden ($\mathbf{h}$) and cell ($\mathbf{c}$) states of a 2-layer LSTM.
...
* **Attention Mechanism**: Instead of compressing the sentence into a static vector, the decoder calculates alignment weights $\alpha_{t, i}$ for all encoder hidden states $\mathbf{h}_i$ at each step $t$:
...
You can interact with the live model directly on the cloud via our **[Streamlit Web App](https://seq2seq-french-to-english.streamlit.app/)**.
